import{t as y}from"./toast-Cbd37M37.js";/* empty css               */import{l as w}from"./logger-CXzo1yMG.js";async function U(t,e="Text"){try{await navigator.clipboard.writeText(t),y.success(`${e} copied to clipboard!`)}catch{const r=document.createElement("textarea");r.value=t,r.style.position="fixed",r.style.left="-999999px",document.body.appendChild(r),r.select();try{document.execCommand("copy"),y.success(`${e} copied to clipboard!`)}catch{y.error(`Failed to copy ${e.toLowerCase()}`)}finally{document.body.removeChild(r)}}}function V(t){const{text:e,type:n,options:r,matching:o,ordering:a,timeLimit:c,points:d}=t;let s="";switch(s+=`<div class="preview-question">${e||"No question text"}</div>`,n){case"mcq":case"poll":s+=Z(r,n==="poll");break;case"tf":s+=K();break;case"fillblank":s+=X(e);break;case"matching":s+=Y(o);break;case"ordering":s+=ee(a);break;default:s+='<div class="alert alert-info">Preview not available for this question type</div>'}return s+=te(c,d,n),s}function Z(t,e=!1){if(!t||t.length===0)return'<div class="alert alert-info">No options added yet</div>';let n='<div class="preview-options">';return t.forEach((r,o)=>{const a=String.fromCharCode(65+o),c=r.trim()||`Option ${a}`;n+=`
            <div class="preview-option">
                <input type="radio" name="preview-answer" value="${o}">
                <div class="preview-option-text"><strong>${a}.</strong> ${c}</div>
            </div>
        `}),n+="</div>",e&&(n+='<div class="alert alert-info" style="margin-top: 1rem;"><i class="fas fa-info-circle"></i> This is a poll question - no correct answer</div>'),n}function K(){return`
    ...
    `}function ee(t){if(!t||t.length===0)return'<div class="alert alert-info">Add items to put in order</div>';let e='<div class="alert alert-info"><i class="fas fa-info-circle"></i> Put these items in the correct order</div>';return e+='<div class="preview-ordering-items">',t.forEach((n,r)=>{const o=n.trim()||`Item ${r+1}`;e+=`
            <div class="preview-ordering-item">
                <i class="fas fa-grip-vertical preview-ordering-handle"></i>
                <div>${o}</div>
            </div>
        `}),e+="</div>",e}function te(t,e,n){let r='<div class="preview-metadata">';t&&(r+=`<div class="preview-meta-item"><i class="fas fa-clock"></i> <strong>Time:</strong> ${t}s</div>`),e&&(r+=`<div class="preview-meta-item"><i class="fas fa-star"></i> <strong>Points:</strong> ${e}</div>`);const o={mcq:"Multiple Choice",poll:"Poll Question",tf:"True/False",fillblank:"Fill in the Blank",matching:"Matching",ordering:"Put in Order"};return n&&o[n]&&(r+=`<div class="preview-meta-item"><i class="fas fa-question-circle"></i> <strong>Type:</strong> ${o[n]}</div>`),r+="</div>",r}function oe(){return`
        /* Preview Modal */
        .preview-modal {
            position: fixed;
            top: 0;
    ...
    `}const _=document.createElement("style");_.textContent=oe();document.head.appendChild(_);let k,T,x,ne,f=[],E=-1,R,N=null,Q={defaultTimeLimit:60,defaultPoints:10,defaultAwardParticipation:!1,defaultShowCorrectAnswer:!0,defaultAllowAnswerChange:!0};document.addEventListener("DOMContentLoaded",function(){const t=new URLSearchParams(window.location.search);if(k=t.get("id"),T=t.get("code"),t.get("type"),!k||!T){y.error("Invalid session parameters"),setTimeout(()=>window.location.href="classes.html",2e3);return}const e=sessionStorage.getItem("pendingQuestions");if(e){const n=JSON.parse(e);f.push(...n),sessionStorage.removeItem("pendingQuestions"),sessionStorage.removeItem("currentQuestionIndex"),O()}else{const n=sessionStorage.getItem("pendingQuestion");if(n){const r=JSON.parse(n);f.push(r),sessionStorage.removeItem("pendingQuestion"),O()}}if(f.length===0){const n=sessionStorage.getItem("questionQueue_"+T),r=sessionStorage.getItem("questionIndex_"+T);n&&(console.log("[Session] Restoring queue from sessionStorage"),f=JSON.parse(n),E=r?parseInt(r):-1,O())}f.length===0&&(console.log("[Session] Attempting to restore queue from backend"),re()),ie(),F(),se(),be()});function ie(){localStorage.getItem("token")||(window.location.href="/login.html")}async function F(){try{const t=await fetch(`https://api-modular.intellaclick.com/api/sessions/code/${T}`,{headers:{Authorization:`Bearer ${localStorage.getItem("token")}`}});if(!t.ok)throw new Error("Failed to load session");x=(await t.json()).session,ne=x.classId,w.debug("Session loaded:",{sessionId:k,sessionCode:T,sessionData:x,hasCurrentQuestion:!!x.currentQuestion}),ae()}catch(t){w.error("Error loading session:",t),y.error("Failed to load session")}}async function re(){try{const t=await fetch(`https://api-modular.intellaclick.com/api/sessions/${k}`,{headers:{Authorization:`Bearer ${localStorage.getItem("token")}`}});if(!t.ok){console.error("[Session] Failed to fetch session data for queue restoration");return}const e=await t.json();e.success&&e.session.questionQueue&&e.session.questionQueue.length>0?(f=e.session.questionQueue,E=e.session.currentQuestionIndex??-1,console.log("[Session] Queue restored from backend:",{queueLength:f.length,currentIndex:E}),O(),y.success(`Restored ${f.length} questions from previous session`)):console.log("[Session] No saved queue found in backend")}catch(t){console.error("[Session] Error restoring queue from backend:",t)}}async function z(){try{if(!(await fetch(`https://api-modular.intellaclick.com/api/sessions/${k}/question-queue`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${localStorage.getItem("token")}`},body:JSON.stringify({questionQueue:f,currentQuestionIndex:E})})).ok){console.error("[Session] Failed to save queue to backend");return}console.log("[Session] Queue saved to backend successfully")}catch(t){console.error("[Session] Error saving queue to backend:",t)}}async function se(){try{const t=await fetch("https://api-modular.intellaclick.com/api/users/question-defaults",{headers:{Authorization:`Bearer ${localStorage.getItem("token")}`}});t.ok?(Q=(await t.json()).defaults||Q,document.getElementById("modalTimeLimit").value=Q.defaultTimeLimit,document.getElementById("modalPoints").value=Q.defaultPoints,document.getElementById("modalAwardParticipation").checked=Q.defaultAwardParticipation,w.debug("✅ Loaded question defaults:",Q)):w.debug("No saved question defaults found, using system defaults")}catch(t){w.error("Error loading question defaults:",t)}}function ae(){document.getElementById("sessionCode").textContent=x.sessionCode,document.getElementById("participantCount").textContent=x.participantCount||0;const t=`https://join.intellaclick.com/session/${x.sessionCode}`;document.getElementById("joinUrl").innerHTML=`
            ...
            `,de(t);const e=document.getElementById("sessionStatus");e.textContent=x.status.charAt(0).toUpperCase()+x.status.slice(1),e.className=`status-badge status-${x.status}`,x.status==="active"&&x.currentQuestion&&(document.getElementById("stopBtn").disabled=!1,document.getElementById("resultsBtn").disabled=!1,D(x.currentQuestion)),le()}function le(){const t=document.getElementById("participantsList");fetch(`https://api-modular.intellaclick.com/api/sessions/${k}/participants`,{headers:{Authorization:`Bearer ${localStorage.getItem("token")}`}}).then(e=>e.json()).then(e=>{e.participants&&e.participants.length>0?(t.innerHTML="",e.participants.forEach(n=>{const r=document.createElement("div");r.className="participant-card",r.innerHTML=`
